...
loop do
  ...
  updates = files.find_all { |file, last_changed|
    File.mtime(file) > last_changed
  }
  changed_file, last_changed = updates[0]

  updated_files = updates.map {|p| p[0]}
  
  if changed_file
    
    command_with_args = command.gsub(/\{\}/, updated_files.join(" "))
    
    files[changed_file] = File.mtime(changed_file)
    puts "=> #{changed_file} changed, running #{command_with_args}"
    results = "" # `#{command}`
